...
layer_state_t layer_state_set_user(layer_state_t state) {
    //レイヤによるスクロールモードの変更
    g_charybdis_config.scrolling_mode = 0;
     if (get_highest_layer(state) == 1){
        g_charybdis_config.scrolling_mode = 3;
    } else if (get_highest_layer(state) == 3){
        g_charybdis_config.scrolling_mode = 2;
    }
    return state;   
}
...
static void pointing_device_task_charybdis(report_mouse_t* mouse_report) {
    ...
    if (g_charybdis_config.scrolling_mode==1) {
...
#    endif  // CHARYBDIS_DRAGSCROLL_REVERSE_Y
    ...
    } else if (g_charybdis_config.scrolling_mode==2) {
        //horizontal only
#    ifdef CHARYBDIS_DRAGSCROLL_REVERSE_Y
        scroll_buffer_y -= mouse_report->y;
#    else
...
#    endif  // CHARYBDIS_DRAGSCROLL_REVERSE_Y
    ...
    } else if (g_charybdis_config.scrolling_mode==3){
        //vertical only
        #    ifdef CHARYBDIS_DRAGSCROLL_REVERSE_X
        scroll_buffer_x -= mouse_report->x;
#    else
